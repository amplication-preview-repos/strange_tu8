/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { LandingPageWhereUniqueInput } from "./LandingPageWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { LandingPageUpdateInput } from "./LandingPageUpdateInput";

@ArgsType()
class UpdateLandingPageArgs {
  @ApiProperty({
    required: true,
    type: () => LandingPageWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => LandingPageWhereUniqueInput)
  @Field(() => LandingPageWhereUniqueInput, { nullable: false })
  where!: LandingPageWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => LandingPageUpdateInput,
  })
  @ValidateNested()
  @Type(() => LandingPageUpdateInput)
  @Field(() => LandingPageUpdateInput, { nullable: false })
  data!: LandingPageUpdateInput;
}

export { UpdateLandingPageArgs as UpdateLandingPageArgs };
